	...
	if object_array.size() != 0:
		for desk in object_array:
			if desk.object_name == "ReceptionDesk" && desk.is_occuped == true:
				desk_ptr = desk
				pathfinding = pathfinding_res.getPath(Vector2(get_translation().x, get_translation().z), desk.vector_pos, self)
	...
	if object_array.size() != 0:
		for drinking in object_array:
			if drinking.object_name == "DrinkMachine":
				pathfinding = pathfinding_res.getPath(Vector2(get_translation().x, get_translation().z), drinking.vector_pos, self)
				add_child(pathfinding)
				return
	...
	if object_array.size() != 0:
		for bench in object_array:
			if bench.object_name == "Bench" && bench.is_occuped == false:
				bench_ptr = bench
				pathfinding = pathfinding_res.getPath(Vector2(get_translation().x, get_translation().z), bench.vector_pos, self)
				add_child(pathfinding)
